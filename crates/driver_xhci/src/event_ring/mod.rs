...
impl EventRing {
    pub fn new(r: Registers<_>) -> Self {
        ...
        Self {
            ...
            rings: Vec![VecDeque::from([[u32, 4]; MAX_NUM_OF_TRB_IN_QUEUE]); erst_num],
            counter: 0,
            index: 0,
            erst_num,
            flipsign: true,
    ...
    pub fn cycle(&mut self) -> Option<Allowed> {
        ...
        let sign = bool::from(next.get_bit(0));
        match sign == self.flipsign {
            false => None,
            true => {
                let allowed = Allowed::from(next);
                if allowed.is_err() {
    ...
    pub fn update_deq_p_with_xhci(&self, mut r: Registers<_>) {
    ...
    pub fn rings_addr(&self) -> Vec<usize> {
